...
export const listDimensoesCanonicas = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<PainelDims> => {
    ...
    const [sec, uni, gc, car, esp, vin, mot] = await Promise.all([
      ...
      supabase.from("dim_cargo").select("id,nome_oficial,grupo_cargo_id,especialidade_id"),
    ...
    return {
      ...
      cargos: (car.data ?? []).map((r: any) => ({ id: r.id, nome: r.nome_oficial, parent_id: r.grupo_cargo_id })),
